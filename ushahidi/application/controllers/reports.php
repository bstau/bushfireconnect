...
#		$this->template->content->categories = $this->_get_categories($form['incident_category']);
		$this->template->content->categories = $this->_get_subcategories($form['incident_category']);		
		// Javascript Header
    ...
    /*
    * Modification _get_subcategoires
    */
    private function _get_subcategories($selected_categories)
	{
		// Count categories to determine column length
		$subcategories_query = "category_visible = 1 AND parent_id != 0";

		$categories_total = ORM::factory('category')
                            ->where($subcategories_query)
                            ->count_all();
